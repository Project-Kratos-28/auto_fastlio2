...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1),
  angle_filter_en(false), blind_front_deg(0.0), blind_back_deg(0.0)
{
  inf_bound = 10;
  N_SCANS = 6;
  SCAN_RATE = 10;
          ...
          ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
              ...
              && (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z > (blind * blind)))
          {
            if (angle_filter_en)
            {
              double yaw_deg = atan2(pl_full[i].y, pl_full[i].x) * 57.29577951308232;
              if (fabs(yaw_deg) <= (blind_front_deg * 0.5) || fabs(yaw_deg) >= (180.0 - blind_back_deg * 0.5))
              {
                continue;
              }
            }
